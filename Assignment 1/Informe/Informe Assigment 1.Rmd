...
Para obtener el k más óptimo se realiza un barrido con diferentes k hasta obtener los mejores resultados En la figura de la derecha se puede observar el barrido del parámetro k. 


Se puede observar que el valor de accurancy más alto se obtiene cuando el k=2. También podemos observar que el valor de k más alto que hemos probado es 40, porque se ve claramente que el accurancy tiene una línea descendente cuanto más alto es el k. Que el valor seccionado sea $k=2$ puede ocasionar un sobre entrenamiento y por ello vemos también el valor Kappa que en este caso es $\text{Kappa}=0.5225$.
...
El modelo de árboles de decisión consiste en realizar divisiones recursivas. Para ello se analiza la mejor variable para dividir el conjunto de datos y se van obteniendo diferentes ramificaciones que permiten clasificar cada uno de los datos. El número de divisiones que se ejecutan son los diferentes nodos del árbol. En estos modelos el hiperparámetro que se utiliza es uno llamado *cp* que sirve para penalizar a los árboles que tienen una altura mayor. Si este parámetro tuviera el valor 0, el árbol tendría un gran número de nodos y el modelo puede llegar a estar sobre entrenado.
Por ello, realizaremos un barrido de este parámetro yendo desde 0 hasta 0.1 en pasos de 0.001.
...
Se puede ver que el valor más alto es $cp = 0.004$, que genera un $Accurancy=0.78$. Tras hacer la evaluación de los resultados, obtenemos que para el conjunto de test el $Accurancy=0.77$, muy parecido al conjunto de entrenamiento. Evaluando los resultados más de cerca, tenemos la matriz de confusión:
...
Además de esta matriz, vamos a utilizar el indicador de la curva ROC. A continuación vemos la gráfica de la curva ROC para el conjunto de test. El área bajo la curva tiene un valor de 0.8215 lo que no está mal.


\begin{figure}
\centering
\includegraphics[width=50mm]{ROC DT Test 1.png}
\caption{ROC DC Test 1}
\end{figure} 
...
Se puede ver en la Figura 8 que el valor más alto es $cp = 0$, que genera un valor de $Accurancy=0.78$, muy parecido a los que generaba el modelo sin optimizar. Si evaluamos el conjunto de test, vemos que el $Acurrancy=0.77$, también muy parecido al anterior modelo. Por lo tanto, a pesar de que el parámetro $cp=0$ el modelo no parece estar sibreentrenado. Evaluando los resultados más de cerca, tenemos la matriz de confusión:
\begin{table}[htb]
\centering
\begin{tabular}{|l|l|l|}
\hline
...
Además de esta matriz, vamos a utilizar el indicador de la curva ROC. A continuación vemos la gráfica de la curva ROC para el conjunto de test. El área bajo la curva tiene un valor de 0.8201 lo que es aceptable.

\begin{figure}[htb]
\centering
\includegraphics[width=60mm]{ROC DT Test 2.png}
\caption{ROC DC Test 2}
\end{figure} 
...
Estos resultados generan un $Accurancy= 0.73$. Además de esta matriz, vamos a utilizar el indicador de la curva ROC. A continuación vemos la gráfica de la curva ROC para el conjunto de test. El área bajo la curva tiene un valor de 0.819 lo que es aceptable.

\begin{figure}[htb]
\centering
\includegraphics[width=60mm]{ROC SVM Lineal.png}
\caption{ROC SVM Lineal}
\end{figure} 
...
En este modelo no solo tenemos que modificar el parámetro C como ocurría en el SVM Lineal, sino que también tenemos que ver el parámetro Sigma. Para obtener una correcta configuración hemos establecido un rango de ambos parámetros y hemos realizado otro barrido. Lo vemos en la Figura 13.

\begin{figure}[htb]
\centering
\includegraphics[width=90mm]{params SVM Radial.png}
\caption{Params SVM Radial}
\end{figure}

Se puede ver que los diferentes valores de sigma convergen cuando C es grande. Por este motivo vamos a coger la línea que siempre está por encima y que representa el valor $sigma=450$ y $C=14.5$
Evaluando los resultados del conjunto de test tenemos la matriz de confusión:
\begin{table}[htb]
\centering
\begin{tabular}{|l|l|l|}
\hline
...
Estos resultados generan un $Accurancy=0.85$, lo que mejora enormemente los resultados del modelo anterior SVM Lineal.
...
\begin{figure}[htb]
\centering
\includegraphics[width=60mm]{ROC SVM Radial.png}
...
\centering
\includegraphics[width=60mm]{curvarocrnopt}
\caption{Curvas ROC}
\end{wrapfigure}
...
La configuración óptima para este modelo son $neuronas=12$ y $decay=1e-09$ De esta forma obtenemos un $Accurancy= $ para el conjunto de entrenamiento. Y evaluando el conjunto de Test, tenemos un resultado de $Accurancy= $. Si lo comparamos con los 2 modelos de Redes Neuronales anteriores vemos que se comparta bastante peor que ellos. 

\begin{figure}[h]
\centering
\includegraphics[width=90mm]{Comparativa NN}
